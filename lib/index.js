...
const OPENCODE_USAGE_TTL_MS = 30000
...
if (window.__dshWhaleWidget) {
  console.warn('[whale-opencode] 原版 dsh-whale-widget 已启用，OpenCode Go 版本未激活。如需使用 OpenCode Go 用量，请禁用原版插件。')
  return
}
...
usageSelect.appendChild(soundOpt('opencode', 'OpenCode Go 套餐'))
...
function render() {
  ...
  if (costBubbleActive) return
  var amount, hint
  // reset label/amount/hint styles (incl. OpenCode Go 的字号覆写) for non-opencode modes
  if (!state.opencodeWindows) {
    labelEl.textContent = 'DeepSeek 余额'
    labelEl.className = 'dshwv-label'
    labelEl.style.color = ''
  ...
  if (state.status === 'error') {
  ...
  } else if (state.opencodeWindows) {
    // OpenCode Go mode: 3 lines
    // line1 (labelEl): "OpenCode Go" (big)
    // line2 (amountEl): "5h 1.0% | 周 0.0% | 月 21.0%" (dark, small)
    // line3 (hintEl): "2h55m | 6d22h | 20d8h" (dark, small)
    labelEl.textContent = 'OpenCode Go'
    labelEl.style.color = ''
    // OpenCode Go 三行整体收小一号，避免字形顶到气泡边框
    labelEl.style.fontSize = 'calc(var(--dshw-u) * 84)'
    labelEl.className = 'dshwv-period'
    var rw = state.opencodeWindows.rolling
    var ww = state.opencodeWindows.weekly
    var mw = state.opencodeWindows.monthly
    // line2: percentages with tier labels
    var pParts = []
    if (rw) pParts.push('5h ' + rw.percent.toFixed(1) + '%')
    if (ww) pParts.push('周 ' + ww.percent.toFixed(1) + '%')
    if (mw) pParts.push('月 ' + mw.percent.toFixed(1) + '%')
    amount = pParts.length ? pParts.join(' | ') : '--'
    amountEl.className = 'dshwv-hint'
    amountEl.style.color = '#536ba9'
    amountEl.style.fontSize = 'calc(var(--dshw-u) * 44)'
    hintEl.style.fontSize = 'calc(var(--dshw-u) * 44)'
    // line3: reset times aligned with percentages
    var resetParts = []
    function fmtReset(resetsAt) {
      if (!resetsAt) return '--'
      var ms = new Date(resetsAt).getTime() - Date.now()
      if (ms <= 0) return '刷新中'
    ...
    if (rw) resetParts.push(fmtReset(rw.resetsAt))
    if (ww) resetParts.push(fmtReset(ww.resetsAt))
    if (mw) resetParts.push(fmtReset(mw.resetsAt))
    hint = resetParts.length ? resetParts.join(' | ') : ''
...
function refresh(manual) {
  ...
  fetch(BALANCE_URL, { cache: 'no-store', signal: ctrl ? ctrl.signal : undefined })
    ...
    .then(function (data) {
      if (data && data.ok) {
        // OpenCode Go mode
        if (data.usageMode === 'opencode' && data.opencodeWindows) {
          state.opencodeWindows = data.opencodeWindows
          state.balance = null
          state.currency = null
          state.message = ''
...
function setUsageMode(v) {
  usageMode = v === 'token' ? 'token' : v === 'opencode' ? 'opencode' : 'ledger'
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    ...
    if (d && typeof d.usageMode === 'string') {
      usageMode = d.usageMode === 'token' ? 'token' : d.usageMode === 'opencode' ? 'opencode' : 'ledger'
      usageSelect.value = usageMode
    }
...
const name = 'whale-balance-widget-opencode'
...
function apply(ctx) {
    ...
    // OpenCode Go usage: uses OPENCODE_GO_API_KEY to query official usage API
    async function fetchOpenCodeGoUsage() {
      let cred
      try {
        cred = await ctx.credentials.resolve('OPENCODE_GO_API_KEY')
      } catch (err) {
        return { ok: false, code: 'NO_KEY', error: '凭据读取失败: ' + String((err && err.message) || err).slice(0, 160) }
      }
      if (!cred) {
        return { ok: false, code: 'NO_KEY', error: '未配置 OPENCODE_GO_API_KEY' }
      }
      const key = String(cred.value)
      try {
        ...
        if (!res.ok) return { ok: false, code: 'HTTP', error: 'HTTP ' + res.status }
        const data = await res.json()
        if (data && data.error) {
          return { ok: false, code: 'API', error: String(data.error.message || data.error.type || '接口返回错误') }
        }
        const usage = data && data.usage ? data.usage : null
        if (!usage || typeof usage !== 'object') {
          return { ok: false, code: 'SHAPE', error: '响应缺少 usage 字段' }
        }
        const windows = {}
        for (const name of ['rolling', 'weekly', 'monthly']) {
          ...
          if (w && typeof w === 'object') {
            windows[name] = {
              ...
              limitUsd: OPENCODE_LIMITS_USD[name] || null,
            }
          }
        }
        if (!windows.rolling && !windows.weekly && !windows.monthly) {
          return { ok: false, code: 'SHAPE', error: '响应缺少用量窗口字段' }
        }
        return { ok: true, windows }
      } catch (err) {
        return { ok: false, code: 'ERROR', error: String((err && err.message) || err).slice(0, 200) }
      }
    }
    ...
    async function getBalancePayload() {
      ...
      // OpenCode Go mode: fetch from OpenCode API directly
      if (mode === 'opencode') {
        const ocResult = await fetchOpenCodeGoUsage()
        if (ocResult.ok) {
          return {
            ok: true,
            usageMode: 'opencode',
            opencodeWindows: ocResult.windows,
            isPeak: isPeakTime(Math.floor(Date.now() / 1000)),
          }
        }
        // OpenCode fetch failed: fall through to DeepSeek mode
      }

      // DeepSeek modes (ledger / token)
