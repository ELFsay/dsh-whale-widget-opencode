...
export default {
  name: 'whale-balance-widget-opencode',
  ...
  apply(ctx) {
    ...
    function handleSessionEvent(sessionId, event) {
      try {
        ...
        // 兜底：万一某 provider 上报的 reasoning 不是 output 的子集（reasoning > output），
        ...
        agg.cost += costMsg
        if (model) {
          agg.byModel[model] = (agg.byModel[model] || 0) + costMsg
    ...
    function usageRecordsPayload() {
      ...
      function modelsFor(d) {
        ...
        arr.forEach((it) => { it.cost = round2(it.cost) })
        // 今天若有“未入明细”的真实消费(事件漏记部分),补一行使合计对得上
        if (d === today) {
          const gap = tot - evSum
          if (gap > 0.004) arr.push({ model: '(未入明细)', cost: round2(gap) })
        }
      ...
      return {
        ok: true,
        today: { total: totalDay(today), models: modelsFor(today) },
        days7: days7,
        ...
        if (m.id === API_BUILTIN_ID) {
          ...
          try {
            ...
            if (p && p.ok) {
              ...
              entry.todayUsage = ledgerTodayTotal(readUsageLedger())
              entry.todayUsageCurrency = 'CNY' // 内置 DeepSeek 走账本，一律人民币
    ...
    // 记账：跨天时以当天第一次观测到的余额为“当日零点起始余额 dayStart”；
    // 当天内每次观测都滚动刷新 lastBalance,不再因“今天已有事件”而提前返回——
    // 事件明细可能因会话中断/重启漏记,真实余额差(dayStart−lastBalance)始终可算。
    // 归档以真实余额差为准(与官网一致,事件计价偏高时不被带高);余额差不可用时才用事件。
    ...
    async function getBalancePayload() {
      ...
      full.todayUsage = ledgerTodayTotal(led)
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            const result = writeSizeConfig(scale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.scrollGapOn, parsed.scrollGapPx, parsed.menuBtnHide)
    ...
    disposers.push(registerRoute({
      ...
      handler: async (req, res) => {
        try {
          // 与余额路由同源:请求到来时若持有较新的余额缓存(≤70s,覆盖两次 60s 轮询间隙),
          // 先用它刷新账本 lastBalance 再算汇总,保证“今日合计”与泡泡“今日已用”读同一份数据;
